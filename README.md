...
flowchart LR
    ...
    E4 -.-> CAPABILITIES
```
Я бы выбрал Grafana потому UI удобнее =) ну и дашборды отличаются , в Grafana они функциональнее в реальном времени и с динамическими переменными справляются лучше чем в Kibana.
